...
Codeunit 10091 "Export Payments (RB)"
{
    ...
    var
        ...
        Text004: label 'Cannot export details until an export file is started.';
        Text015: label 'Either %1 or %2 must refer to either a %3 or a %4 for an electronic payment.';
        Text016: label 'Vendor No. %1 has no bank account setup for electronic payments.';
        Text017: label 'Vendor No. %1 has more than one bank account setup for electronic payments.';
        Text018: label 'Customer No. %1 has no bank account setup for electronic payments.';
        Text019: label 'Customer No. %1 has more than one bank account setup for electronic payments.';
        Text005: label 'Cannot end export file until an export file is started.';
        Text006: label 'File %1 does not exist.';
        Text047: label 'Did the transmission work properly?';
        ...
        Text020: label 'Demand Debits are not supported. Check sign on %1 %2, %3 %4, %5 %6.';
        Text021: label 'You must now run the program that transmits the payments file to the bank. Transmit the file named %1 located at %2 to %3 (%4 %5 %6).  After the transmission is completed, you will be asked if it worked correctly.  Are you ready to transmit (answer No to cancel the transmission process)?';
    ...
    procedure StartExportFile(BankAccountNo: Code[20];ReferenceCode: Code[10];GenJnlLine: Record "Gen. Journal Line")
    var
    ...
    local procedure AddToPrnString(var PrnString: Text[250];SubString: Text[80];StartPos: Integer;Length: Integer;Justification: Option Left,Right;Filler: Text[1])
    var
        I: Integer;
        SubStrLen: Integer;
    begin
