...
Codeunit 5052 AttachmentManagement
{
    ...
    begin
        ...
        repeat
          ...
          if TempDeliverySorterOther."Correspondence Type" = TempDeliverySorterOther."correspondence type"::Email then begin
            ...
            AttachmentFileFullName := PrepareClientAttachment(
                TempSubDirPath,TempDeliverySorterOther."Attachment No.");

            // Send the mail
            Contact.Get(InteractLogEntry."Contact No.");
            SendEmailWithAttachment(
              TempDeliverySorterOther,InteractLogEntry,AttachmentFileFullName);
            // Clean up
    ...
    local procedure SendEmailWithAttachment(TempDeliverySorterOther: Record "Delivery Sorter" temporary;InteractLogEntry: Record "Interaction Log Entry";AttachmentFileFullName: Text)
    var
        Mail: Codeunit Mail;
        IsSent: Boolean;
    begin
        IsSent :=
          Mail.NewMessage(
            InteractionEMail(InteractLogEntry),'','',TempDeliverySorterOther.Subject,'',
            AttachmentFileFullName,false);
    ...
    local procedure PrepareClientAttachment(var TempSubDirPath: Text;AttachmentNo: Integer): Text
    var
        Attachment: Record Attachment;
    ...
    begin
        ...
        TempFileFullName := FileManagement.ClientTempFileName('');
        Attachment.ExportAttachmentToClientFile(TempFileFullName);
        TempSubDirPath := FileManagement.CreateClientTempSubDirectory;
        AttachmentFileFullName := FileManagement.CombinePath(TempSubDirPath,AttachmentTok + '.' + Attachment."File Extension");
        FileManagement.MoveFile(TempFileFullName,AttachmentFileFullName);
        ...
        if DeliverySorter.Find('-') then begin
          ...
          repeat
            ...
            case true of
              ...
              WordManagement.IsWordDocumentExtension(Attachment."File Extension"):
                begin
