...
Page 70083 "ACA-Pending Admissions PSSP"
{
    ...
    SourceTableView = where(Status=filter("Provisional Admission"),
                            ...
                            "Settlement Type"=filter(PSSP|SSP|"PSSP-PG"));
                ...
                action(GenerateAdmissionNo)
                {
                    ...
                    begin
                        ...
                        if AdminSetup.Find('-') then   begin
                          if (("Settlement Type"='KUCCPS') or ("Settlement Type"='GOK') or ("Settlement Type"='JAB')) then
                            settlementPrefix:=AdminSetup."JAB Prefix"
                            else if  (("Settlement Type"='PSSP') or ("Settlement Type"='SSP') or ("Settlement Type"='SELFSPONSORED')) then
