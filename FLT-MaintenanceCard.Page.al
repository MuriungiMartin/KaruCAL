...
Page 55519 "FLT-Maintenance Card"
{
    DeleteAllowed = false;
    PageType = Card;
    ...
    layout
    {
        area(content)
        {
            ...
            group(ServiceDetails)
            {
                ...
                {
                    ApplicationArea = Basic;
                    Caption = 'Service Done By (Supplier)';
                }
                ...
                {
                    ApplicationArea = Basic;
                    Description = 'Date of Service/Maint/Repair';
                    OptionCaption = 'Date of Service/Maint/Repair';
                }
    ...
    begin
        Clear(DocPending);
        Clear(DocumentClosed);
        Clear(DocumentApproved);
        Clear(DocumentOveralEditable);
        if ((Rec.Status=Rec.Status::New) or (Rec.Status=Rec.Status::"Pending Approval")) then DocPending:=true;
        if (Rec.Status=Rec.Status::Closed) then DocumentClosed:=true;
        if (Rec.Status=Rec.Status::Approved) then DocumentApproved:=true;
        if DocPending then begin
           DocumentOveralEditable:=true;
          DocumentApproved:=false;
          end;
          if DocumentClosed then begin
           DocumentOveralEditable:=false;
            DocumentApproved:=false;
          end;
    ...
    var
        ...
        DocumentOveralEditable: Boolean;
}
