...
Report 51220 "SPV Pending Approval"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/SPV Pending Approval.rdlc';

    dataset
    {
        dataitem(UnknownTable61134;UnknownTable61134)
        {
            DataItemTableView = sorting(No) where(Status=const(Pending),"Payment Type"=const(Salary),Posted=const(No),"Approved By"=const("<>"));
            RequestFilterFields = Date,"Pay Mode","IW No";
            ...
            column(SPVs_Pending_ApprovalCaption;SPVs_Pending_ApprovalCaptionLbl)
            {
            }
    ...
    var
        SPVs_Pending_ApprovalCaptionLbl: label 'SPVs Pending Approval';
