...
Report 51214 "Claims Pending Payments"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Claims Pending Payments.rdlc';

    dataset
    {
        dataitem(UnknownTable61134;UnknownTable61134)
        {
            DataItemTableView = sorting(No) where(Status=const("2nd Approval"),"Payment Type"=const(Claims));
            ...
            column(Claims_Pending_PaymentsCaption;Claims_Pending_PaymentsCaptionLbl)
            {
            }
    ...
    var
        Claims_Pending_PaymentsCaptionLbl: label 'Claims Pending Payments';
