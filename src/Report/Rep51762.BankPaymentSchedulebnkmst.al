...
Report 51762 "Bank Payment Schedule-bnk mst"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Bank Payment Schedule-bnk mst.rdlc';
    ...
    dataset
    {
        dataitem(UnknownTable61077;UnknownTable61077)
        {
            ...
            RequestFilterFields = "Bank Code","Branch Code","Bank Name";
            column(ReportForNavId_4233; 4233)
            {
            }
            column(GETFILTERS__________HR_Employee__GETFILTERS;GetFilters + ' ' + "HRM-Employee (D)".GetFilters)
            {
            }
            ...
            column(HR_Employee__Account_Type_Caption;"HRM-Employee (D)".FieldCaption("Account Type"))
            {
            }
            dataitem(UnknownTable61118;UnknownTable61118)
            {
                ...
                column(HR_Employee__HR_Employee___Net_Pay_;empTrans.Amount)
                {
                }
                ...
                column(HR_Employee__HR_Employee___No__;"HRM-Employee (D)"."No.")
                {
                }
                column(HR_Employee__Account_Type_;"Account Type")
                {
                }
                column(HR_Employee__HR_Employee___Net_Pay__Control1102755001;empTrans.Amount)
                {
                }
                ...
                column(DepartmentCode_HREmployee;"HRM-Employee (D)"."Department Code")
                {
                }
                ...
                begin
                    ...
                    empTrans.Reset;
                    empTrans.SetRange(empTrans."Employee Code","HRM-Employee (D)"."No.");
                    empTrans.SetRange(empTrans."Transaction Code",'NPAY');
                    empTrans.SetRange(empTrans."Payroll Period",PeriodFilter);
                    if empTrans.Find('-') then begin
                    if empTrans.Amount = 0 then
                    CurrReport.Skip;



                    GrandTotal:=GrandTotal+empTrans.Amount;
                ...
                begin
                    ...
                    //"HR-Employee".SETFILTER("HR-Employee"."Period Filter",'=%1',PeriodFilter);
                    //IF "HR-Employee".FIND('-') THEN
                end;
            }
    ...
    var
        ...
        empTrans: Record UnknownRecord61092;
}
