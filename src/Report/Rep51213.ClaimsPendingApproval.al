...
Report 51213 "Claims Pending Approval"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Claims Pending Approval.rdlc';

    dataset
    {
        dataitem(UnknownTable61134;UnknownTable61134)
        {
            DataItemTableView = sorting(No) where("Payment Type"=const(Claims),Posted=const(No),Status=const(Pending),"Approved By"=const());
            ...
            column(Claims_Pending_ApprovalCaption;Claims_Pending_ApprovalCaptionLbl)
            {
            }
    ...
    var
        Claims_Pending_ApprovalCaptionLbl: label 'Claims Pending Approval';
