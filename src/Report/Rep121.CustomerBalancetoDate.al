...
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Customer - Balance to Date.rdlc';
    ...
    dataset
    {
        dataitem(Customer;Customer)
        {
            ...
            dataitem(CustLedgEntry3;"Cust. Ledger Entry")
            {
                ...
                begin
                    Reset;
                    FilterDetailedCustLedgEntry(DtldCustLedgEntry,StrSubstNo('%1..%2',CalcDate('<-1D>',MaxDate),99991231D));
    ...
    local procedure FilterDetailedCustLedgEntry(var DetailedCustLedgEntry: Record "Detailed Cust. Ledg. Entry";DateFilter: Text)
    begin
        with DetailedCustLedgEntry do begin
          SetCurrentkey("Customer No.","Posting Date","Entry Type");
          SetRange("Customer No.",Customer."No.");
          SetFilter("Posting Date",DateFilter);
          SetRange("Entry Type",DtldCustLedgEntry."entry type"::Application);
          if not PrintUnappliedEntries then
    ...
    local procedure CalcCustomerTotalAmount()
    var
    ...
    begin
        with TempCustLedgerEntry do begin
          FilterDetailedCustLedgEntry(DetailedCustLedgEntry,'');
          if DetailedCustLedgEntry.FindSet then
            repeat
              CustLedgerEntry.Get(DetailedCustLedgEntry."Cust. Ledger Entry No.");
              if not Get(CustLedgerEntry."Entry No.") then begin
                TempCustLedgerEntry := CustLedgerEntry;
                Insert;
              end;
            until DetailedCustLedgEntry.Next = 0;

          FilterCustLedgerEntry(CustLedgerEntry);
