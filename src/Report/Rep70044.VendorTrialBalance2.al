...
Report 70044 "Vendor - Trial Balance 2"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Vendor - Trial Balance 2.rdlc';
    ...
    dataset
    {
        dataitem(Vendor;Vendor)
        {
            ...
            RequestFilterFields = "No.","Date Filter","Vendor Posting Group","Is a Medical Claim";
            ...
            begin
                CalcAmounts(
                  ...
                  PeriodBeginBalance,PeriodDebitAmt,PeriodCreditAmt,YTDTotal,isMedicalClaim);
                CalcAmounts(
                  FiscalYearStartDate,PeriodEndDate,
                  YTDBeginBalance,YTDDebitAmt,YTDCreditAmt,YTDTotal,isMedicalClaim);
            end;
            ...
            begin
                ...
                if Evaluate(isMedicalClaim,Vendor.GetFilter("Is a Medical Claim")) then;
            end;
        }
    }
    ...
    var
        ...
        DetailedVendLedger: Record "Detailed Vendor Ledg. Entry";

    local procedure CalcAmounts(DateFrom: Date;DateTo: Date;var BeginBalance: Decimal;var DebitAmt: Decimal;var CreditAmt: Decimal;var TotalBalance: Decimal;IsaMedicalClaims: Boolean)
    var
    ...
    begin
        ...
        VendorCopy.SetRange("Is a Medical Claim",IsaMedicalClaims);
        if VendorCopy.Find('-') then;
    ...
    local procedure GetTotals(VendorId: Code[20];IsMedClaim: Boolean)
    var
        VendLedgers: Record "Detailed Vendor Ledg. Entry";
        CreditAmounts: Decimal;
        DebitAmounts: Decimal;
    begin
        Clear(VendLedgers);
        //Sum("Detailed Vendor Ledg. Entry"."Debit Amount (LCY)" WHERE (Vendor No.=FIELD(No.),
        //Entry Type=FILTER(<>Application),
        //Initial Entry Global Dim. 1=FIELD(Global Dimension 1 Filter),
        //Initial Entry Global Dim. 2=FIELD(Global Dimension 2 Filter),
        //Posting Date=FIELD(Date Filter),
        //Currency Code=FIELD(Currency Filter),
        //Is a Medical Claim=FIELD(Is a Medical Claim)))

        VendLedgers.Reset;
        VendLedgers.SetRange("Vendor No.",VendorId);
        VendLedgers.SetFilter("Posting Date",PeriodFilter);
        VendLedgers.SetFilter("Entry Type",'<>%1',VendLedgers."entry type"::Application);
        VendLedgers.SetFilter("Posting Date",PeriodFilter);
        VendLedgers.SetFilter("Posting Date",PeriodFilter);
        if Vendor.GetFilter("Is a Medical Claim") <> '' then begin
          VendLedgers.SetFilter("Is a Medical Claim",Vendor.GetFilter("Is a Medical Claim"));
          end;
          if VendLedgers.Find('-') then begin
            repeat
              begin

              end;
              until VendLedgers.Next = 0;
            end;
    end;
}
