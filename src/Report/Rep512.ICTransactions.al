...
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/IC Transactions.rdlc';
    ...
    dataset
    {
        dataitem(HeaderInt;"Integer")
        {
            ...
            column(ReportForNavId_4514; 4514)
            {
            }
            ...
            dataitem(ICPartner1;"IC Partner")
            {
                ...
                column(ReportForNavId_4280; 4280)
                {
                }
            ...
            dataitem(ICPartner2;"IC Partner")
            {
                ...
                PrintOnlyIfDetail = true;
                column(ReportForNavId_9189; 9189)
                {
                }
                ...
                dataitem(Customer;Customer)
                {
                    ...
                    {
                        DataItemTableView = sorting(Number) order(ascending) where(Number=const(1));
                        column(ReportForNavId_9436; 9436)
                        {
                        }
            ...
            dataitem(ICPartner3;"IC Partner")
            {
                DataItemTableView = sorting(Code) order(ascending);
                PrintOnlyIfDetail = true;
                column(ReportForNavId_8237; 8237)
                {
                }
                ...
                dataitem(Vendor;Vendor)
                {
                    ...
                    {
                        DataItemTableView = sorting(Number) order(ascending) where(Number=const(1));
                        column(ReportForNavId_6109; 6109)
                        {
                        }
    ...
    var
        ...
        TempGLEntry__VAT_Amount__Control20CaptionLbl: label 'Tax Amount';
        ...
        BalanceAmount_Control62CaptionLbl: label 'Balance ($)';
        Customer_LedgerCaptionLbl: label 'Customer Ledger';
        Text002Lbl: label 'Beginning balance';
        BalanceAmount_Control97CaptionLbl: label 'Balance ($)';
