...
Page 1007 "Job Planning Lines"
{
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                {
                    ApplicationArea = All;
                    ToolTip = 'Specifies the number of the job that the planning line relates to.';
        ...
        area(navigation)
        {
            group("Job Planning &Line")
            {
                ...
                action("&Reservation Entries")
                {
                    ...
                    ToolTip = 'View all reservations that are made for the item on the job planning line, either by you manually or automatically.';
    ...
    procedure SetJobNoVisible(NewJobNoVisible: Boolean)
    begin
        JobNoVisible := NewJobNoVisible;
    end;
