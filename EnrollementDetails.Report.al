...
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Enrollement Details.rdlc';

    dataset
    {
        dataitem(UnknownTable61532;UnknownTable61532)
        {
            DataItemTableView = sorting(Programme) where(Reversed=const(No),Blocked=const(0),"Total Billed"=filter(>0),"Total Paid"=filter(>0));
            RequestFilterFields = Programme,"Academic Year","Semester Filter","Intake Code","Settlement Type",Stage,"Programme Exam Category","Student Status";
