...
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Students Registration-Mass Rev.rdlc';

    dataset
    {
        dataitem(UnknownTable61532;UnknownTable61532)
        {
            DataItemTableView = sorting(Programme,Stage) where(Reversed=filter(No),"Cust Exist"=filter(>0),Balance=filter(>1),Status=filter(Current));
