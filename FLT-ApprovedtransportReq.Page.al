...
Page 69124 "FLT-Approved transport Req"
{
    ApplicationArea = Basic;
    CardPageID = "FLT-Submitted Transport-sb";
    DeleteAllowed = false;
    InsertAllowed = false;
    ModifyAllowed = false;
    PageType = List;
        ...
        area(processing)
        {
            group("&Functions")
            {
                ...
                action("Mark as Closed")
                {
                    ...
                    trigger OnAction()
                    begin
                        if Confirm('Mark requisition as closed?',false)=false then Error('Cancelled by user!');
