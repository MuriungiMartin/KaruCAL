...
Page 859 "Cash Flow Manual Expenses"
{
    ...
    layout
    {
        area(content)
        {
            repeater(Control1000)
            {
                ...
                {
                    ApplicationArea = Basic,Suite;
                    ToolTip = 'Specifies the total amount in $ that the manual expense consists of. The entered amount is the amount that is registered in the given time period per recurring frequency.';
                }
    ...
    begin
        ...
        if not CashFlowManualExpense.FindLast then
          CashFlowCode := ExpTxt + '000001'
        else
          CashFlowCode := IncStr(CashFlowManualExpense.Code);
