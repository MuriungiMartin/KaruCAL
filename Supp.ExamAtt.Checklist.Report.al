...
Report 78086 "Supp. Exam Att. Checklist"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Supp. Exam Att. Checklist.rdlc';

    dataset
    {
        dataitem(Coregsz1;UnknownTable61532)
        {
            CalcFields = Names;
            DataItemTableView = where(Reversed=filter(No));
            PrintOnlyIfDetail = true;
            RequestFilterFields = "Academic Year",Programme,Stage;
            ...
            {
                DataItemLink = Programme=field(Programme),"Student No."=field("Student No."),Semester=field(Semester);
                DataItemTableView = where(Category=filter(Supplementary|Special));
                RequestFilterFields = "Unit Code";
                column(ReportForNavId_1000000014; 1000000014)
                {
                }
                column(SemCode;"Aca-Special Exams Details".Semester)
                {
                }
                dataitem(StudUnits;UnknownTable61549)
                {
                    DataItemLink = Programme=field(Programme),"Student No."=field("Student No."),Unit=field("Unit Code"),Semester=field(Semester);
                    DataItemTableView = sorting("Student No.",Unit) order(ascending) where("Reg Reversed"=filter(No));
                    ...
                    begin
                          "Units/Subj".Reset;
                          "Units/Subj".SetRange("Units/Subj"."Programme Code",Coregsz1.Programme);
                ...
                var
                    Valid: Boolean;
                begin
                    Valid:=("Aca-Special Exams Details".Status="Aca-Special Exams Details".Status::Approved)or("Aca-Special Exams Details"."Approval Status"="Aca-Special Exams Details"."approval status"::Approved);
                    if not Valid then
                      CurrReport.Skip();
                end;
            }
            ...
            begin
                Names:='';
                Prog.Reset;
                Prog.SetRange(Code,Coregsz1.Programme);
                if Prog.Find('-') then;
                DValue.Reset;
