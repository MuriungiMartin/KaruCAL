...
Codeunit 8614 "Config. XML Exchange"
{
    ...
    var
        ...
        TableContainsRecordsQst: label 'Table %1 in package %2 contains %3 records that will be overwritten by the import. Do you want to continue?';
        MissingInExcelFileErr: label '%1 is missing in the Excel file.';
        Text014: label 'Exporting package';
        Text017: label 'Importing package';
        PackageFileNameTxt: label 'Package%1.rapidstart', Locked=true;
        Text027: label 'Download';
        ...
        ExcelMode: Boolean;
        HideDialog: Boolean;
        Text031: label 'DataList', Locked=true;
        ...
        RecordProgressTxt: label 'Import %1 records';
    ...
    local procedure ApplyPackageFilter(ConfigPackageTable: Record "Config. Package Table";var RecRef: RecordRef)
    var
    ...
    procedure ImportPackageXMLDocument(PackageXML: dotnet XmlDocument): Boolean
    var
        ConfigPackage: Record "Config. Package";
    ...
    local procedure GetNodeBase64StringValue(var RecordNode: dotnet XmlNode;FieldNodeName: Text[250]): Text
    var
        FieldNode: dotnet XmlNode;
    begin
        FieldNode := RecordNode.SelectSingleNode(FieldNodeName);
    ...
    begin
        if ConfigPackageMgt.IsBLOBField(ConfigPackageData."Table ID",ConfigPackageData."Field ID") and not ExcelMode then begin
          TempBlob.FromBase64String(GetNodeBase64StringValue(RecordNode,FieldNodeName));
          ConfigPackageData."BLOB Value" := TempBlob.Blob;
        end else
          ConfigPackageData.Value := GetNodeValue(RecordNode,FieldNodeName);
    end;
